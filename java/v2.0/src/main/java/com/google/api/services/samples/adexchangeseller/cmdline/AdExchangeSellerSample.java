...
public class AdExchangeSellerSample {
  ...
  public static void main(String[] args) {
    try {
      ...
      Accounts accounts = GetAllAccounts.run(adExchangeSeller, MAX_LIST_PAGE_SIZE);
      if ((accounts.getItems() != null) && !accounts.getItems().isEmpty()) {

        String exampleAccountId = accounts.getItems().get(0).getId();
        System.out.printf("Using %s as the sample account id.%n", exampleAccountId);

        AdClients adClients = GetAllAdClients.run(adExchangeSeller, exampleAccountId,
            MAX_LIST_PAGE_SIZE);
        if ((adClients.getItems() != null) && !adClients.getItems().isEmpty()) {
          ...
          CustomChannels channels =
              GetAllCustomChannels.run(adExchangeSeller, exampleAccountId,
                  exampleAdClientId, MAX_LIST_PAGE_SIZE);
          if ((channels.getItems() != null) && !channels.getItems().isEmpty()) {

          }

          GetAllUrlChannels.run(adExchangeSeller, exampleAccountId,
              exampleAdClientId, MAX_LIST_PAGE_SIZE);
          GenerateReport.run(adExchangeSeller, exampleAccountId,
              exampleAdClientId);
          GenerateReportWithPaging.run(adExchangeSeller, exampleAccountId,
              exampleAdClientId, MAX_REPORT_PAGE_SIZE);
        } else {
          System.out.println("No ad clients found, unable to run remaining methods.");
        }

        SavedReports savedReports = GetAllSavedReports.run(adExchangeSeller,
            exampleAccountId, MAX_REPORT_PAGE_SIZE);
        if ((savedReports.getItems() != null) && !savedReports.getItems().isEmpty()) {
          ...
          GenerateSavedReport.run(adExchangeSeller, exampleAccountId, exampleSavedReportId);
        } else {
          System.out.println("No saved report found.");
        }

        GetAllDimensions.run(adExchangeSeller, exampleAccountId);
        GetAllMetrics.run(adExchangeSeller, exampleAccountId);

        GetAllAlerts.run(adExchangeSeller, exampleAccountId);

        GetAllPreferredDeals.run(adExchangeSeller, exampleAccountId);
      }
    } catch (IOException e) {
