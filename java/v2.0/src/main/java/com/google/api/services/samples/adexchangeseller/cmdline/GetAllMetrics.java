...
public class GetAllMetrics {
  ...
  public static void run(AdExchangeSeller adExchangeSeller, String accountId) throws Exception {
    System.out.println("=================================================================");
    System.out.println("Listing all metrics for the specified account");
    System.out.println("=================================================================");

    // Retrieve and display metrics.
    Metadata metrics = adExchangeSeller.accounts().metadata().metrics().list(accountId).execute();
    ...
    if (metrics.getItems() != null && !metrics.getItems().isEmpty()) {
      for (ReportingMetadataEntry metric : metrics.getItems()) {
        ...
        System.out.printf("Metric id \"%s\" for product(s): [%s] was found.%n",
